...
class LpMarkdown extends LocalizedWidget {
  /// Themed [Markdown] widget.
  const LpMarkdown(this.data, {Key? key})
      : source = null,
        assert(data != null),
        super(key: key);
  ...
  const LpMarkdown.network(this.source, {Key? key})
      : data = null,
        assert(source != null),
        super(key: key);
      ...
      trueWidget: (_) => FutureBuilder<http.Response>(
        ...
        builder: (context, snapshot) {
          if (snapshot.hasData) {
            ...
            if (snapshot.data!.statusCode == 200) return _buildMarkdown(context, snapshot.data!.body);
